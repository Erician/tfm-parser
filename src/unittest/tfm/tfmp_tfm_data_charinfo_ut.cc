...
class TfmCharInfoTest : public ::testing::Test
{
protected:
    virtual void SetUp() {
        ;
    ...
    tfmp::tfm::data::CharInfo char_info;
};

TEST_F(TfmCharInfoTest, Parse_ShouldPass)
{
    ...
    ASSERT_EQ(0, char_info.Parse(header.bc_, header.ec_, tfm_ifs));
}

TEST_F(TfmCharInfoTest, Show_ShouldPass)
{
    std::ifstream *tfm_ifs = new std::ifstream();
    tfm_ifs->open("./cmr10.tfm", std::ifstream::in);
    ASSERT_EQ(0, header.Parse(tfm_ifs));
