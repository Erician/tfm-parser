...
#ifndef UTILS_STRINGUTIL_
#include "utils/stringutil.h"
#endif

#ifndef LOGGING_CONST_
#include "logging/const.h"
#endif

Config::Config()
{
    config_items_[config::kLogFilePath] = "";
}
...
bool Config::Init(std::string config_file_path)
{
    ...
    while(! ifs.eof())
    {
        ...
        if(line.length() == 0 || line[0] == '\n' || line[0] == '#')
            continue;
        line = utils::stringutil::strip(line, '\n');
        std::vector<std::string> pairs = utils::stringutil::split(line, "=");
        for(std::vector<std::string>::iterator it = pairs.begin(); it != pairs.end(); it++)
            *it = utils::stringutil::strip(*it, ' ');
